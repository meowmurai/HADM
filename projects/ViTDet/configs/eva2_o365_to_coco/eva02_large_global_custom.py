"""
Training config for HADM-G (Global) on a custom dataset.

This config fine-tunes from a pretrained HADM-G checkpoint on your custom
...
Usage:
  python tools/lazyconfig_train_net.py \
      --config-file projects/ViTDet/configs/eva2_o365_to_coco/eva02_large_global_custom.py \
      --num-gpus=1 \
      train.eval_period=2000 \
      train.log_period=100 \
      train.output_dir=./outputs/eva02_large_global_custom \
      dataloader.evaluator.output_dir=cache/large_global_custom_val \
      dataloader.train.total_batch_size=4

  To fine-tune from existing HADM-G weights (recommended):
...
dataloader.train.dataset = L(get_detection_dataset_dicts)(
    names=[
        "global_human_artifact_custom_train",
    ],
    filter_empty=False)
...
# 12 global artifact classes
model.roi_heads.num_classes = 12
dataloader.train.total_batch_size = 4

# Fine-tune from pretrained HADM-G weights (recommended)
train.init_checkpoint = "pretrained_models/HADM-G_0249999.pth"

# Validation on custom val split
dataloader.test.dataset.names = "global_human_artifact_custom_val"
dataloader.evaluator = L(COCOEvaluator)(
    dataset_name="global_human_artifact_custom_val",
    output_dir="./cache/large_global_custom_val",
)
